...
Install the single `bin/tako` multicall executable, systemd units from `apps/backend/packaging/systemd`, `sysusers.d` for the `tako-session` group, PAM policy from `apps/backend/packaging/pam`, and read-only branding assets under `/usr/share/tako/branding`. Systemd socket-activates `tako serve` as a dynamic unprivileged gateway (`tako-gateway`) and `tako sessiond` as the PAM/host-authority boundary. Both exit after the configured idle interval when no sessions or work remain; their sockets stay active for the next connection. `tako bridge` is launched per login with the authenticated user's credentials. These remain isolated processes with distinct users and sandboxes; no gateway process retains root privileges.
