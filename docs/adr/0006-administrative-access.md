...
Service targets use one shared parser in the platform domain. The gateway validates system/user scope, bounded unit names, and the fixed lifecycle action set before opening the session socket; sessiond repeats the validation at the trust boundary. User-scope actions require the authenticated UNIX bridge token, while system-scope actions require both that binding and an unexpired Administrative grant. Failure responses are stable operation codes and do not expose command output.
