...
Packaging uses one multicall executable with `serve`, `sessiond`, and `bridge` modes. This reduces installed artifacts without combining trust domains: systemd still launches each mode as a separate process with its own user, lifecycle, file descriptors, and sandbox policy.
